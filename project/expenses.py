...
def get_today_statistics() -> str:
    ...
    if not result[0]:
        return "There were no expenses today"
...
def get_month_statistics() -> str:
    ...
    result = cursor.fetchone()
    if not result[0]:
        return "There were no expenses in this month"
    all_today_expenses = result[0]
    cursor.execute(f"select sum(amount) "
                   f"from expense where date(created) >= '{first_day_of_month}' "
    ...
    result = cursor.fetchone()
    base_today_expenses = result[0] if result[0] else 0
    return (f"Monthly expenses:\n"
            f"total — {all_today_expenses} rub.\n"
            f"base — {base_today_expenses} rub. from "
...
def _parse_message(raw_message: str) -> Message:
    ...
    if not regexp_result or not regexp_result.group(0) \
        ...
        raise exceptions.NotCorrectMessage(
            "Can't understand. The message should be in the following format, "
            "e.g.:\n1500 subway")
